...
def validate_collection(
    ...
    if use_movie_lookup and movie_ids:
        ...
        if not tmdb_collection:
            print("   No collection found from movies, trying name search as fallback...")
    ...
    if extra_movies:
        ...
        print("      These movies may not belong to this collection:")
    ...
    if not missing_movies and not extra_movies:
        print("   ✅ Collection is complete and accurate!")
...
def main():
    ...
    if args.update:
        print("⚠️  UPDATE MODE: Will add collectionId, enrich all movies with TMDB data, and add missing movies")
    if args.collection:
