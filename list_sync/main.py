...
_current_sync_session_id: str | None = None
    ...
    try:
        ...
        if session_id:
            try:
                ...
                logging.exception(f"Error updating database in signal handler: {e}")
...
def check_cancellation_requested(session_id: str | None = None) -> bool:
    """
...
def handle_cancellation(sync_tracker, session_id: str | None = None):
    """
...
def fetch_media_from_lists(
    list_ids: list[dict[str, str]], is_single_list: bool = False
    ...
    for item in all_media:
        ...
        if imdb_id:
            if imdb_id not in seen_imdb_ids:
                ...
                existing_keys = [
                    f"{l['type']}:{l['id']}:{l.get('user_id','1')}" for l in existing_item["_source_lists"]
                ]
...
def get_source_lists_from_item(
    item: dict[str, Any], list_type: str | None = None, list_id: str | None = None
) -> list[dict[str, str]]:
    """
    ...
    try:
        ...
        if tmdb_id:
            ...
            if search_result:
                ...
                logging.info("✅ SUCCESS: Direct TMDB ID lookup")
        ...
        if not search_result and imdb_id and tmdb_api.is_available():
            ...
            if tmdb_match:
                ...
                logging.info(
                    f"✅ TMDB resolved IMDB {imdb_id} → TMDB {resolved_tmdb_id} " f"('{tmdb_match.get('title')}')",
                )
                search_result = seerr_client.get_media_by_tmdb_id(resolved_tmdb_id, resolved_type)
                if search_result:
                    match_method = "IMDB_TO_TMDB_DIRECT"
                    logging.info("✅ SUCCESS: IMDB→TMDB direct lookup")
        ...
        if not search_result:
            ...
            if search_result:
                ...
                logging.warning("⚠️  SUCCESS: Fallback title search (may be less accurate)")
        ...
        if search_result:
            ...
            if source_lists:
                ...
                logging.error(
                    f"❌ CRITICAL: No source lists found for item '{title}'! _source_lists={item.get('_source_lists')}, _source_list_type={item.get('_source_list_type')}, _source_list_id={item.get('_source_list_id')}, list_type={list_type}, list_id={list_id}"
                )
            ...
            logging.info("🚀 STATUS: Requesting media...")
            statuses = []
            for user_id in users_to_request:
                if search_result["mediaType"] == "tv":
                    ...
                    if season_number is not None:
                        ...
                        request_status = seerr_client.request_specific_season(
                            overseerr_id, season_number, is_4k, requester_user_id=user_id
                        )
                    else:
                        logging.info(f"📺 TV SERIES: Requesting {number_of_seasons} season(s) as user {user_id}")
                        request_status = seerr_client.request_tv_series(
                            overseerr_id, number_of_seasons, is_4k, requester_user_id=user_id
                        )
                else:
                    logging.info(f"🎬 MOVIE: Submitting request as user {user_id}")
                    request_status = seerr_client.request_media(
                        overseerr_id, search_result["mediaType"], is_4k, requester_user_id=user_id
                    )
            ...
            if "success" in statuses:
                ...
                if failed:
                    ...
                    logging.info("✅ SUCCESS: Request submitted successfully!")
                final_status = "requested"
            elif "already_requested" in statuses:
                logging.info("📌 STATUS: Already requested (detected from API response)")
...
def sync_media_to_overseerr(
    ...
    if sequential_mode:
        ...
        for i, item in enumerate(media_items, 1):
            ...
            if check_cancellation_requested():
                logging.warning(
                    f"⚠️ Cancellation detected during sequential processing at item {i}/{sync_results.total_items}"
                )
        ...
        for batch_num in range(total_batches):
            ...
            for i, item in enumerate(batch_items):
                ...
                try:
                    ...
                    logging.info(f"{'='*80}")
                    logging.info(
                        f"✅ COMPLETED ITEM {start_idx + i + 1}/{sync_results.total_items} - Status: {status.upper()}"
                    )
                    logging.info(f"{'='*80}\n")
                    ...
                    if check_cancellation_requested():
                        logging.warning(
                            f"⚠️ Cancellation detected after item {start_idx + i + 1}/{sync_results.total_items}"
                        )
                        handle_cancellation(get_sync_tracker(), session_id)
                        sync_results.cancelled = True
                        return sync_results
                    ...
                    logging.exception(f"❌ ERROR PROCESSING ITEM {start_idx + i + 1}/{sync_results.total_items}: {e!s}")
                    logging.exception(f"{'='*80}\n")
                    sync_results.results["error"] += 1
                    current_item += 1
...
def automated_sync(
        ...
        try:
            ...
            if not force_full_sync:
                ...
                if os.path.exists(sync_requests_dir):
                    ...
                    for request_file in request_files:
                        try:
                            ...
                            logging.exception(f"Error reading queued sync request {request_file}: {e}")
                ...
                if os.path.exists(single_list_request_file):
                    try:
                        with open(single_list_request_file) as f:
                            request_data = json.load(f)
                        ...
                        logging.exception(f"Error reading legacy sync request file: {e}")
                ...
                for sync_req in queued_syncs:
                    ...
                    try:
                        ...
                        try:
                            ...
                            logging.exception(
                                f"Error processing queued single list sync for {list_type}:{list_id}: {e!s}"
                            )
                        ...
                        logging.exception(f"Error processing sync request: {e}")
            ...
            if single_list_sync and single_list_type and single_list_id:
                ...
                try:
                    ...
                    logging.exception(f"Error in single list sync: {e!s}")
                    return False
            ...
            logging.exception(f"Error in sync operation: {e!s}")
    ...
    while True:
        try:
            ...
            logging.exception(f"Error in automated sync loop: {e!s}")
    ...
    try:
        ...
        for list_info in synced_lists:
            try:
                update_list_sync_info(list_info["type"], list_info["id"], list_info["item_count"])
                logging.info(
                    f"Updated sync info for {list_info['type']} list {list_info['id']}: {list_info['item_count']} items"
                )
            except Exception as e:
                logging.warning(f"Failed to update sync info for {list_info['type']} list {list_info['id']}: {e}")
        ...
        logging.exception(f"Full sync failed for session {session_id}: {e}")
        ...
        try:
            ...
            logging.exception(f"Could not mark failed sync as ended: {db_error}")
        raise
    ...
    try:
        ...
        try:
            ...
            if not media_items:
                ...
                logging.info("Single list sync completed - no items found")
                # Mark sync as ended in database
                end_sync_in_db(session_id=session_id, status="no_items")
                return result
