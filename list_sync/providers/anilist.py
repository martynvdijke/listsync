...
def fetch_anilist_animelist_graphql(username: str, status_filter: str | None = None) -> list[dict[str, Any]]:
    ...
    try:
        ...
        logging.exception(f"❌ AniList API HTTP error: {e}")
        if e.response.status_code == 404:
            raise ValueError(f"AniList user '{username}' not found")
        raise
    except requests.exceptions.RequestException as e:
        logging.exception(f"❌ AniList API request failed: {e}")
        raise
    except Exception as e:
        logging.exception(f"❌ AniList API error: {e}")
        raise
...
def extract_media_from_anilist_entry(entry: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        logging.warning(f"Failed to extract media from AniList entry: {e!s}")
        return None
