...
def fetch_simkl_watchlist(media_type: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        if "401" in str(e):
            logging.exception("❌ SIMKL API authentication failed. Please check your SIMKL_USER_TOKEN.")
            logging.exception("   Get your OAuth access_token at: https://simkl.com/oauth/authorize")
            logging.exception("   See documentation: https://simkl.com/settings/developer/")
        else:
            logging.exception(f"❌ SIMKL API error fetching {media_type}: {e!s}")
        return []
    except Exception as e:
        logging.exception(f"❌ SIMKL API unexpected error fetching {media_type}: {e!s}")
        return []
...
def search_simkl_by_title(title: str, year: int | None = None, media_type: str = "movie") -> dict[str, Any] | None:
    ...
    try:
        ...
        logging.exception(f"❌ SIMKL API search error: {e!s}")
        return None
    except Exception as e:
        logging.exception(f"❌ SIMKL API unexpected search error: {e!s}")
        return None
