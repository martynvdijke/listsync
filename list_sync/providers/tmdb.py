...
def _fetch_tmdb_list_api(list_id: str, api_key: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        if total_pages > 1:
            ...
            for page in range(2, total_pages + 1):
                try:
                    ...
                    logging.warning(f"Failed to fetch page {page}: {e!s}")
                    continue
        ...
        logging.exception(f"TMDB API request failed: {e!s}")
        raise
    except Exception as e:
        logging.exception(f"Error fetching TMDB list via API {list_id}: {e!s}")
        raise
...
def _process_tmdb_api_item(item: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        logging.warning(f"Failed to process TMDB API item: {e!s}")
        return None
...
def _fetch_tmdb_list_scraping(list_id: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        logging.exception(f"Error fetching TMDB list {list_id}: {e!s}")
        raise
...
def _process_tmdb_list(sb, url) -> list[dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            logging.warning(f"Could not find content with primary selector: {e!s}")
            # Try fallback
            try:
                sb.wait_for_element_present('div[class*="table-row"]', timeout=5)
                logging.info("Found content with fallback selector")
            except Exception as e2:
                raise ValueError(f"Could not find list content on page: {e2!s}")

        # Additional wait to ensure everything is loaded
        sb.sleep(3)

    except Exception as e:
        logging.exception(f"Failed to load TMDB list page: {e!s}")
        raise
    ...
    for i, item in enumerate(items):
        try:
            ...
            logging.warning(f"Failed to parse TMDB item {i+1}: {e!s}")
            continue
    ...
    while pagination_attempt < max_pagination_attempts and no_progress_count < 5:
        ...
        try:
            ...
            logging.warning(f"Error during scroll loading: {e!s}")
        ...
        if load_more_buttons:
            ...
            if button.is_displayed() and button.is_enabled():
                ...
                try:
                    ...
                    logging.warning(f"Error clicking Load More button: {e!s}")
                    no_progress_count += 1
            else:
        ...
        if current_item_count <= previous_item_count:
            ...
            try:
                ...
                logging.warning(f"Error checking page position: {e!s}")
        else:
