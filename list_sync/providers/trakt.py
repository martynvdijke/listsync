...
def log_trakt_error_details(
    ...
    try:
        ...
        masked_headers = {"error": f"Unable to retrieve request headers: {e!s}"}
...
def extract_media_from_list_item(item: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        logging.warning(f"Failed to parse Trakt list item: {e!s}")
        return None
...
def parse_special_list_url(url_or_shortcut: str) -> str:
    ...
    if ":" in url_or_shortcut and not url_or_shortcut.startswith(("http://", "https://")):
        ...
        if len(parts) == 2:
            ...
            if media_type.lower() in ["shows", "show", "tv"]:
...
def extract_media_from_special_list_item(item: dict[str, Any], endpoint: str) -> dict[str, Any] | None:
    """
    ...
    try:
        ...
        logging.warning(f"Failed to parse special list item: {e!s}")
        return None
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if returned_imdb_id and returned_imdb_id != imdb_id:
                ...
                logging.error("🚨 This indicates a Trakt API bug or data corruption. Skipping this result.")
                return None
            ...
            logging.exception(f"❌ Error searching Trakt by IMDB ID {imdb_id}: {e!s}")
            if attempt < max_retries:
                continue
            return None
...
def get_trakt_metadata(
    ...
    try:
        ...
        logging.exception(f"Error fetching Trakt metadata: {e!s}")
        return None
...
def search_trakt_by_title(title: str, year: int | None, media_type: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        logging.exception(f"❌ Error searching Trakt by title '{title}': {e!s}")
        return None
