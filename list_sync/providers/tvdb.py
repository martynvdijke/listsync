...
def _get_tvdb_token(api_key: str) -> str | None:
    ...
    try:
        ...
        logging.exception(f"TVDB API authentication failed: {e!s}")
        return None
    except Exception as e:
        logging.exception(f"Error during TVDB authentication: {e!s}")
        return None
...
def _fetch_tvdb_list_api(list_id: str, api_key: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        logging.exception(f"Error fetching TVDB list {list_id}: {e!s}")
        raise
    ...
    try:
        ...
        for favorite in favorites:
            try:
                ...
                logging.warning(f"Failed to process favorite {favorite}: {e!s}")
                continue

        return media_items

    except requests.exceptions.RequestException as e:
        logging.exception(f"Failed to fetch TVDB user favorites: {e!s}")
        raise
    except Exception as e:
        logging.exception(f"Error processing TVDB user favorites: {e!s}")
        raise
...
def _get_tvdb_series_details(series_id: str, headers: dict[str, str]) -> dict[str, Any] | None:
    ...
    try:
        ...
        logging.warning(f"Failed to get series details for {series_id}: {e!s}")
        return None
    except Exception as e:
        logging.warning(f"Error getting series details for {series_id}: {e!s}")
        return None
...
def _process_tvdb_series_item(series: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        logging.warning(f"Failed to process TVDB series item: {e!s}")
        return None
...
def _process_tvdb_list(sb, url) -> list[dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            logging.warning(f"Could not find TVDB list content: {e!s}")
            return media_items
        ...
        for i, item in enumerate(items):
            try:
                ...
                logging.warning(f"Failed to parse TVDB item {i+1}: {e!s}")
                continue
        ...
        logging.exception(f"Error processing TVDB list: {e!s}")
        return media_items
...
def _enhance_with_tvdb_api(media_items: list[dict[str, Any]], headers: dict[str, str]) -> list[dict[str, Any]]:
    ...
    for item in media_items:
        try:
            ...
            logging.warning(f"Failed to enhance item {item.get('title', 'Unknown')}: {e!s}")
