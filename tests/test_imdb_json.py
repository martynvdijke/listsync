...
def page(payload, script_id="__NEXT_DATA__"):
    return (
        b'<html><body>only a few rendered</body><script id="'
        + script_id.encode()
...
legacy = {
    "list": {
        "items": [
            {"const": "tt0111161", "primaryTitle": "The Shawshank Redemption", "year": 1994, "titleType": "movie"},
            {"const": "tt0903747", "primaryTitle": "Breaking Bad", "startYear": "2008", "titleType": "tvSeries"},
        ]
    }
}
...
dupes = {
    "a": [{"id": "tt0145487"}],
    "b": [{"id": "tt0145487", "titleText": {"text": "Spider-Man"}, "releaseYear": {"year": 2002}}],
}
...
legacy_html = (
    b"<html><script>window.IMDbReactInitialState = "
...
# --- URL resolution ---------------------------------------------------------
check("watchlist url", _resolve_imdb_url("ur171928620"), ("https://www.imdb.com/user/ur171928620/watchlist", False))
check("list url", _resolve_imdb_url("ls123456789"), ("https://www.imdb.com/list/ls123456789", False))
check("chart url", _resolve_imdb_url("top"), ("https://www.imdb.com/chart/top", True))
check(
    "passthrough url", _resolve_imdb_url("https://www.imdb.com/list/ls999/"), ("https://www.imdb.com/list/ls999", False)
)
try:
