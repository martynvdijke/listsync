...
check("  real webhook preserved", stored("discord_webhook"), "https://discord.com/api/webhooks/123/abcdef")
...
check("  and stores nothing new", stored("discord_webhook"), "https://discord.com/api/webhooks/123/abcdef")
requests.get = record_get
...
outbound.clear()
r = client.post(
    "/api/notifications/test", json={"service": "gotify", "url": "http://169.254.169.254/", "token": "apptoken"}
)
...
outbound.clear()
r = client.post(
    "/api/notifications/test", json={"service": "gotify", "url": "http://192.168.1.50:8080", "token": "apptoken"}
)
check("test endpoint attempts a private Gotify url", outbound, [("POST", "http://192.168.1.50:8080/message")])
