...
check(
    "metadata blocked despite allow_private",
    allowed("http://169.254.169.254/latest/meta-data/", allow_private=True),
    False,
)
check(
    "ecs metadata blocked despite allow_private",
    allowed("http://169.254.170.2/v2/credentials", allow_private=True),
    False,
)
check(
    "gcp metadata hostname blocked",
    allowed("http://metadata.google.internal/computeMetadata/v1/", allow_private=True),
    False,
)

# --- private allowed only when asked ---
check("private refused by default", allowed("http://seerr:5055/api/v1/status"), False)
check("private allowed when asked", allowed("http://seerr:5055/api/v1/status", allow_private=True), True)
check("real user's seerr url works", allowed("http://seerr:5055", allow_private=True), True)
...
check("metadata by name, private allowed", allowed("http://ecs.example/", allow_private=True), False)

# --- credentials and malformed ---
check("embedded credentials", allowed("https://user:pass@example.com/"), False)
check("credentials disguising host", allowed("https://discord.com@evil.example/"), False)
...
check("real discord webhook", allowed("https://discord.com/api/webhooks/123/abc", **D), True)
check("discord subdomain", allowed("https://ptb.discord.com/api/webhooks/1/x", **D), True)
...
check("metadata with allowlist refused", allowed("http://169.254.169.254/", **D), False)
...
check(
    "issue #79 PoC blocked", allowed("http://attacker-controlled-domain/", allowed_hosts=DISCORD_WEBHOOK_HOSTS), False
)
...
check("assert returns url when ok", assert_safe_url("https://example.com/a.jpg"), "https://example.com/a.jpg")
check("assert strips whitespace", assert_safe_url("  https://example.com/a.jpg  "), "https://example.com/a.jpg")
